...
import { Activity, Server, Globe, Boxes, ShieldCheck, AlertCircle, Loader2, Bell, TrendingUp, Zap, Radio, Database, Cpu, History } from 'lucide-react';
...
export default function MissionControl() {
  ...
  return (
    <div className="min-h-screen bg-[#010101] text-slate-400 font-sans selection:bg-indigo-500/30 overflow-x-hidden p-4 md:p-8">
      ...
      {/* 1. TOP: GLOBAL HEALTH BAR (SRE-Grade) */}
      <header className="max-w-[1600px] mx-auto mb-8 grid grid-cols-2 md:grid-cols-5 gap-px bg-slate-800/30 border border-slate-800/50 rounded-2xl overflow-hidden shadow-2xl">
         ...
         <HeaderTile label="Error Rate (RED)" value={`${g.error_rate}%`} color={parseFloat(g.error_rate) > 1 ? 'text-rose-500' : 'text-slate-400'} icon={<AlertCircle size={14}/>} />
         <HeaderTile label="Traffic (RPS)" value={g.rps} color="text-blue-400" icon={<Zap size={14}/>} />
         <HeaderTile label="SLO Compliance" value={`${g.slo}%`} color={parseFloat(g.slo) < 99.9 ? 'text-amber-500' : 'text-emerald-400'} icon={<History size={14}/>} />
      </header>
      ...
      <main className="max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
         ...
         <div className="lg:col-span-8 space-y-8">
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
               <TrendBox label="Processor Saturation" value={`${data?.infra?.cpu}%`} history={data?.infra?.cpu_history} color="indigo" />
               <TrendBox label="Network I/O Path" value={`${data?.infra?.net_in}MB/s`} history={[]} color="blue" />
            </section>
               ...
               <div className="overflow-x-auto">
                  <table className="w-full text-left">
                     ...
                     <tbody className="divide-y divide-slate-800/30 font-mono">
                        {data?.apps?.length > 0 ? data.apps.map((app: any, i: number) => (
                           ...
                           <tr><td colSpan={5} className="px-6 py-12 text-center text-[10px] font-black text-slate-700 uppercase tracking-widest">Awaiting PM2 Telemetry Sync...</td></tr>
                        )}
                     </tbody>
                  </table>
               </div>
                  ...
                  {data?.websites?.map((site: any, i: number) => (
                     <div key={i} className="bg-[#080808] p-5 flex items-center justify-between group hover:bg-emerald-500/[0.02] transition-colors">
                        ...
                        <div className="flex gap-6 items-center">
                           ...
                           <div className="text-right">
                              <div className="text-[8px] font-black text-slate-600 uppercase tracking-widest mb-0.5">SLO Status</div>
         ...
         <div className="lg:col-span-4 space-y-8">
            ...
            <section className="bg-indigo-500/[0.03] border border-indigo-500/10 rounded-2xl p-6 shadow-xl">
               ...
               <div className="space-y-5 font-mono">
                  <InfoItem icon={<Server size={12}/>} label="Node Type" value="SRE-V5 PRODUCTION" />
